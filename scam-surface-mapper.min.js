...
function hasCyrLatMix(s)\{return/[A-Za-z]/.test(s)&&/[\uc0\u1040 -\u1071 \u1072 -\u1103 \u1025 \u1105 ]/.test(s);\}\
...
function pushURL(url,meta)\{\
  ...
  var aff=params.filter(([k])=>/(^|)(ref|aff|partner|pid|subid|affiliate)(|$)/i.test(k)).map(x=>x);\
  var trackers=params.filter(([k])=>/(gclid|fbclid|ysclid|yclid|msclkid|cid|clid)/i.test(k)).map(x=>x);\
...
head.innerHTML='<strong>Scam Surface Mapper \'97 Advanced</strong> <span style="margin-left:10px;font-weight:normal" class="ssm_small"> <a href="https://github.com/paulpogoda" target="_blank" style="color:#75a4da;font-weight:normal;">Created by Pavel Bannikov</a> </span><span class="ssm_small" style="margin-left:10px">links: '+records.length+', hosts: '+hostRows.length+'</span><span class="ssm_small" style="margin-left:auto">'+now.toISOString()+'</span>';\
var closeBtn=D.createElement('button');closeBtn.textContent='Close \uc0\u10005 ';closeBtn.type='button';closeBtn.id='ssm_btn';closeBtn.style.marginLeft='10px';closeBtn.onclick=function()\{wrap.remove();\};\

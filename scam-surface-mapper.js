...
var wrap=D.createElement('div');wrap.id='ssm_wrap';
var head=D.createElement('div');head.id='ssm_head';
head.innerHTML='<strong>Scam Surface Mapper — Advanced</strong> <span style="margin-left:10px;font-weight:normal" class="ssm_small"> <a href="https://github.com/paulpogoda" target="_blank" style="color:#75a4da;font-weight:normal;">Created by Pavel Bannikov</a> </span><span class="ssm_small" style="margin-left:10px">links: '+records.length+', hosts: '+hostRows.length+'</span><span class="ssm_small" style="margin-left:auto">'+now.toISOString()+'</span>';
...
var fsOn=false,fsBtn=document.createElement('button');fsBtn.textContent="Fullscreen";fsBtn.id='ssm_btn';fsBtn.style.marginLeft='10px';fsBtn.onclick=function(){fsOn=!fsOn;wrap.style.inset=fsOn?'0':'1%';fsBtn.textContent=fsOn?"Window":"Fullscreen";resizeCanvas();drawGraph();};
...
var bodyBox=D.createElement('div');bodyBox.id='ssm_body';
var side=D.createElement('div');side.id='ssm_side';
var main=D.createElement('div');main.id='ssm_main';
var graphBox=D.createElement('div');graphBox.id='ssm_graph';graphBox.style.display='none';
var canvas=D.createElement('canvas');canvas.id='ssm_canvas';graphBox.appendChild(canvas);
let current='summary';
...
var tSummary=btn('Summary',()=>{current='summary';graphBox.style.display='none';main.style.display='block';renderSummary()});
var tLinks=btn('Raw Links',()=>{current='links';graphBox.style.display='none';main.style.display='block';renderLinks()});
var tGraph=btn('Graph',()=>{current='graph';main.style.display='none';graphBox.style.display='flex';resizeCanvas();drawGraph();});
tabs.append(tSummary,tLinks,tGraph);
/* === HOST-ONLY GRAPH === */
var nodes=[],edges=[],idOf={},nid=0;function addNode(key,obj){if(idOf[key]!=null)return idOf[key];idOf[key]=nid;nodes.push(Object.assign({id:nid},obj));return nid++}
...
var cw,ch,ctx,DPR=W.devicePixelRatio||1;
...
W.addEventListener('mouseup',()=>{dragging=null;});
W.addEventListener('mousemove',e=>{if(!dragging)return;var r=canvas.getBoundingClientRect(),x=e.clientX-r.left,y=e.clientY-r.top,p=pos.get(dragging);p.x=x-dragOff.dx;p.y=y-dragOff.dy;drawGraph();});
...
W.addEventListener('click',()=>{menu.style.display='none';});
resizeCanvas();initPositions();stepLayout(200);drawGraph();
...
bodyBox.appendChild(side);bodyBox.appendChild(main);bodyBox.appendChild(graphBox);
wrap.appendChild(head);wrap.appendChild(tabs);wrap.appendChild(bodyBox);D.body.appendChild(wrap);
function onResize(){if(current==='graph'){resizeCanvas();drawGraph();}}W.addEventListener('resize',onResize);
if(hostRows.length>0){current='graph';main.style.display='none';graphBox.style.display='flex';drawGraph();}
